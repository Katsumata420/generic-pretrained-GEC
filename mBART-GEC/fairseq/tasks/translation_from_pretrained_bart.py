...
class TranslationFromPretrainedBARTTask(TranslationTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--gec_lang', metavar='LANG',
                            help='for GEC language')
        # fmt: on

    def __init__(self, args, src_dict, tgt_dict):
        super().__init__(args, src_dict, tgt_dict)
        self.langs = sorted(args.langs.split(','))
        self.gec_lang = args.gec_lang
        assert self.gec_lang in self.langs
    ...
    def build_generator(self, args):
        if getattr(args, 'score_reference', False):
            ...
            return SequenceScorer(
                ...
                eos=self.tgt_dict.index('[{}]'.format(self.gec_lang))
            )
        else:
            ...
            return SequenceGenerator(
                ...
                eos=self.tgt_dict.index('[{}]'.format(self.args.gec_lang))
            )

    def build_dataset_for_inference(self, src_tokens, src_lengths):
        # src_lang_id = self.source_dictionary.index('[{}]'.format(self.args.source_lang))
        src_lang_id = self.source_dictionary.index('[{}]'.format(self.gec_lang))
        source_tokens = []
